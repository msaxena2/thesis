...
model introduces unnecessary overhead. Since no \emph{instantaneous} rule
can apply until some machine $\mathcal{M}_i \in S$ finishes its sleep duration,
a combined \emph{tick} that advances time by $\textit{min}\left(\tau_i\right)_{0
\leq i < n}$ can be used instead. Second, it precludes us from supporting
