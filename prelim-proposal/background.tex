...
Recall that the \inlinek{<k>} cell typically contains the
...
\Box[-1pt]$\inlinek{ ;
...</k>} by an application of the arithemtic addition rule on line 29. On
line 23, we specify any term of sort \inlinek{Int} to be a \inlinek{KResult}.
This signifies that the term can no longer be evaluted, and is \emph{cooled}
...
As shown in the comparison, not only did the $\K$ gnerated interpreter significantly
