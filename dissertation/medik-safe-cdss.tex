...
Recall from \autoref{sec:semantics-in-k} that execution data in $\K$
is organized into units called emph{cells}, which are labeled may be nested.
\autoref{lst:medik-configuration} shows a simplified version of $\MediK's{}$ configuration.
...
The \inlinek{<instance>} cell
(Lines \ref{lstline:instance-cell-begin}-\ref{lstline:instance-cell-end})
contains state of each \MediK{}
machine instance during execution. Each instance manages its
instance variables using a map in the \inlinek{<genv>} cell
(\autoref{lstline:genv-cell}),
a buffer of incoming events in the \inlinek{<inBuffer>} cell
(\autoref{lstline:in-buffer-cell})
and the currently executing code in the \inlinek{<k>} cell
(Lines \ref{lstline:start-cell}).\footnote{For brevity, we present a simplified version of the configuration. See \cite{medikUrl} for the entire configuration.}
...
machine's entry block. Recall that \scantokens{\lstinline{~>}} symbol
(\autoref{lstline:start-cell})
...
on Lines \ref{lstline:instance-cell-begin}, \ref{lstline:machine-cell-begin}, and
\ref{lstline:interface-cell-begin} signifies that multiple copies of the
corresponding cells, in this case \inlinek{<instance>}, \inlinek{<machine>} and
\inlinek{<interface>} cells can exist in the configuration during execution.
This allows for multiple machine definitions, each with multiple
instances, to exist. We shall discuss interfaces, which are held under the
\inlinek{<interfaces>} cell in upcoming section.
The \inlinek{<machine>} cell (Lines
\ref{lstline:machine-cell-begin}-\ref{lstline:machine-cell-end}) holds information relevant to
