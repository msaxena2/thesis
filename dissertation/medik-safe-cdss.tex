...
concurrently executing communicating state machines.
Next, we described the process of
...
Advanced Cardiac Life Support (\ACLS{}) guidelines for managing
...
content. The semantics-first approach we described in
\autoref{sec:semantics-first}
...
\begin{enumerate*}[label=(\roman*)]
  ...
  described.
\end{enumerate*}
As is typically the case, the \KACLS{} system's functionality
in \autoref{sec:kacls-cdss} was informed by collaboration between
\HCPs{} and computer scientists. As discussed in
\autoref{sec:rewriting-based-guidelines-discussion}, while the \K{}-based
representation of the \ACLS{} guideline was concise,
...
we recall the example \BPG{} from \autoref{sec:bpg-background}
...
In \autoref{sec:medik-psepsis}, we evaluate the effectiveness of our approach
by utilizing it to build a \CDSSs{} intended for real-world use.
In \autoref{sec:discussion}, we discuss how \MediK{} builds on
...
syntax Exp ::= Id                                                         @\label{lstline:value-start}@
             ...
             | "interval" "(" Exp "," Exp ")"
             > Exp "in" Exp                     [macro]               @\label{lstline:case-stmt}@
             | "parseInt" "(" Exp ")"           [strict]
             | StandaloneExp
\end{lstlisting}
...
syntax Stmt ::=
    ...
    | "fun" Id "(" Ids ")" Block
    | NonDetStmt
    | Exp "in" "{" CaseDecl "}"                 [macro-rec] @\label{lstline:case-2-stmt}@
    | StateDecl
    > "machine" Id Block                        [macro]
    | "machine" Id "receives" Ids Block
    | "interface" Id Block                      [macro]    @\label{lstline:interface-declaration-begin}@
    | "interface" Id "receives" Ids Block                  @\label{lstline:interface-declaration-end}@
...
configuration
  <instances>                                                  @\label{lstline:instances-cell-begin}@
  ...
  <machines>                                                   @\label{lstline:medik-machines-cell-begin}@
    <machine multiplicity="*" type="Map">                      @\label{lstline:medik-machine-cell-begin}@
    ...
    </machine>                                                 @\label{lstline:medik-machine-cell-end}@
  </machines>                                                  @\label{lstline:medik-machines-cell-end}@
...
\begin{enumerate}[label=(\arabic*)]
  \item Description of program state expressed as a configuration.
...
\begin{enumerate}[label=(\alph*)]
  ...
  \item A \inlinek{<machines>} cell (Lines \ref{lstline:medik-machines-cell-begin}-\ref{lstline:medik-machines-cell-end})
...
\ref{lstline:instance-cell-begin}, \ref{lstline:medik-machine-cell-begin}, \ref{lstline:interface-cell-begin}),
...
The \inlinek{<machine>} cell (Lines \ref{lstline:medik-machine-cell-begin}-\ref{lstline:medik-machine-cell-end})
...
These cells are utilized in implementing the scheduling strategy \MediK{} uses
that shall be
...
there exists a substitution $\theta$ s.t. $C = \phi\theta$.
...
\noindent In the rule, \inlinek{I}, \inlinek{V}, \inlinek{Loc}, and \inlinek{Store}
...
being executed. Suppose we're executing the statement \inlinek{i = 2;}. In this
...
\begin{itemize}
  ...
  \item On \autoref{lstline:machine-name} to identify the \inlinek{<machine>}
    (Lines \ref{lstline:instance-creation-machine-cell-begin}-\ref{lstline:instance-creation-machine-cell-end})
...
rule <id> SourceId </id>
     ...
     <machine>                                                         @\label{lstline:instance-creation-machine-cell-begin}@
     ...
     </machine>                                                        @\label{lstline:instance-creation-machine-cell-end}@
...
blocks of initial states as such, and typically place code commonly found
...
\subsection{Sending and Receiving Messages}\label{sec:messaging}
...
rule <instance>                                                             @\label{lstlin:instance-a-begin}@
      <k> send instance(RecvId) , EventName:Id , ( Args )                   @\label{lstline:k-send-start}@
      =>  done ...
      </k>                                                                  @\label{lstline:k-send-end}@
       ...
...
\begin{enumerate}[label=(\roman*)]
  \item Obtains the \inlinek{id} of the receiver instance,
    ...
    against the current configuration (Lines \ref{lstline:k-send-start}-\ref{lstline:k-send-end}) respectively.
  ...
  \item The \epoch{} decides when the machine can run and is discussed in
    \autoref{sec:scheduling-semantics}.
\end{enumerate}
...
with transition systems defined outside the \MediK{} program, like other languages, such as Object-Oriented Maude, \cite{OOPMaudeUrl}.
...
the heart rate sensor, we declare it as an interface (\autoref{lstline:hr-interface-decl}).
...
using the \inlinemedik{broadcast} construct described in \autoref{sec:messaging}.

\subsection{Tables}\label{sec:tables}

Recall from \autoref{sec:sepsis-bpg} that tables indicating
normality/abnormality for parameters indexed by age and weight occur
commonly in \BPGs{}. For instance, in case of the sepsis \BPG{} from \autoref{sec:sepsis-bpg},
...
function that does the required check, as shown in \autoref{lst:hr-table}.
\begin{lstlisting}[
  float=ht
  ,frame=single
  ,style=mediksty
  ...
  ,numberstyle=\tiny
  ,xleftmargin=3ex,
  ,caption={{Vital Signs Tables}}
  ,label={lst:hr-table}
  ]
fun isHeartRateNormal() {
  days(age) in {
    interval(days(0)  , months(1)): return hr > 205; @\label{lstline:interval-begin}@
    interval(months(1), months(3)): return hr > 205; @\label{lstline:interval-end}@
    // omitting other cases
    default                       : return hr > 100; @\label{lstline:default-clause}@
  }
}
\end{lstlisting}
While the table in \autoref{lst:hr-table} could have also been implemented using
simple \inlinemedik{if-else} constructs, we chose to present data in form of a table
to improve overall readability and \HCP{} comprehensibility, as \HCPs{} are
already familiar with them from paper-based guidelines.
If the heart rate lies any of the intervals (closed on the left, open on the right) on
Lines \ref{lstline:interval-begin}-\ref{lstline:interval-end}, the
corresponding statement to the right of the colon (\inlinemedik{:}) is run.
Otherwise, the default clause (\autoref{lstline:default-clause}) is run.
We can then define the semantics of tables in terms of \inlinemedik{if-else} as shown in \autoref{lst:table-rules}.
\begin{lstlisting}[
  float=hb
  ,frame=single
  ,style=ksty
  ...
  ,xleftmargin=3ex,
  ,numberstyle=\tiny
  ,caption={{Implementing Tables}}
  ,label={lst:table-rules}
  ]
...
Note the syntax declarations for tables on \autoref{lstline:case-stmt} of \autoref{lst:medik-exp-syntax}
and \autoref{lstline:case-2-stmt} of \autoref{lst:medik-stmt-syntax}
have the attributes \inlinek{[macro]} and \inlinek{[macro-rec]} respectively.
These attributes specify that the corresponding constructs are merely
syntactic sugar, and rules written over such constructs \say{de-sugar}
them into other symbols. Rules that operate over symbols with attribute
\inlinek{[macro]} are applied statically at parse time,
before execution begins, allowing the productions to be used as
...
\section{\MediK{} Scheduling Semantics}\label{sec:scheduling-semantics}

Since the $\K{}$-generated interpreter is single-threaded,
\MediK{} employs interleaving-semantics
...
becomes available. Recall from \autoref{sec:k-rules} that if multiple
$\K$ rules can match the current configuration, then for execution,
$\K$ will choose one non-deterministically. This, in case of a program
comprising of concurrent $\MediK{}$ machines may lead to \emph{unfairness},
...
We show this execution strategy in \autoref{alg:scheduling-semantics}.

%\begin{algorithm}[t]
%  \centering
%    \begin{algorithmic}[1]
%    \State $\epoch{} \gets 0$
%      \State $\scheduled{} \gets \left\{ \Instance_{\Machine{}_{0},0}^{0} \right\}$ \label{algline:init-scheduled}
%    \While{$\scheduled{} \neq \emptyset$}
%       \State \begin{varwidth}[t]{\linewidth}
%         $\Instance_{\Machine_{i},j}^{\tau} \gets \textit{choose}\left(\scheduled{}\right) \text{ s.t. }$ \label{algline:choose}
%           \par
...
%      \State $\textit{execute}(\Instance_{\Machine_{i},j}^{\tau}, \scheduled{})$ \label{algline:execute}
...
%      \newline
%       \State $\epoch{} \gets \epoch{} + 1$ \label{algline:epoch-advance}
%      \EndIf
%    \EndWhile
%  \end{algorithmic}
%  \caption{\MediK{} Scheduling Semantics}\label{alg:scheduling-semantics}
%\end{algorithm}

\begin{algorithm}[t]
    \SetAlgoLined
    \DontPrintSemicolon
    $\epoch{} \longleftarrow 0$\;
    $\scheduled{} \longleftarrow \left\{ \Instance_{\Machine{}_{0},0}^{0} \right\}${\label{algline:init-scheduled}}\;
    \While{$\scheduled{} \neq \emptyset$}{
       $\Instance_{\Machine_{i},j}^{\tau} \longleftarrow \textit{choose}\left(\scheduled{}\right) \text{ s.t. } \tau \leq \epoch{} \wedge \enabled(\Instance_{\Machine_{i},j}^{\tau})${\label{algline:choose}}\;
       $\textit{execute}(\Instance_{\Machine_{i},j}^{\tau}, \scheduled{})${\label{algline:execute}}\;
       \If{$\not\exists i',j',\tau'$ s.t. $\left(\Instance_{\Machine_{i'},j'}^{\tau'} \in \scheduled{}\right) \wedge \left(\tau' \leq \epoch{}\right) \wedge \left(\enabled\left(\Instance_{\Machine_{i'},j'}^{\tau'}\right)\right)$}{
         $\epoch{} \longleftarrow \epoch{} + 1${\label{algline:epoch-advance}}\;
       }
    }
  \caption{\MediK{} Scheduling Semantics}\label{alg:scheduling-semantics}
\end{algorithm}

Recall from \autoref{sec:syntax} that a \MediK{} program consists of a set
...
of the \emph{initial machine} $\Instance_{\Machine_0,0}$ becomes scheduled (\autoref{algline:init-scheduled}) at \epoch{} 0.
On \autoref{algline:choose}, an instance $\Instance_{\Machine_{i},j}^{\tau}$ is
non-deterministically chosen from all machines that are both scheduled to run when
$\tau \leq \epoch{}$ and \textit{enabled}. We use
$\textit{execute}(\Instance_{\Machine_{i},j}^{\tau}, \scheduled{})$ on \autoref{algline:execute}
...
\{\Instance_{\Machine_{i'}, j'}^{\epoch{}+1}\}$. Similarly, if a \inlinemedik{goto}
statement is encountered, the entry block of the target state is scheduled
for execution at \epoch{} + 1. If no other machine is both \textit{scheduled} to run
in the current \epoch{}, and \textit{enabled}, then the \epoch{} advances by one (\autoref{algline:epoch-advance}).

%Consider the rule for
%
%\begin{lstlisting}[
%  frame=ht
%  ,style=ksty
...
%  ,numberstyle=\tiny
%  ,xleftmargin=3ex,
%  ,caption={{Semantics of \inlinemedik{goto}}}
%  ,label={lst:goto-semantics}
%]
%
...
%        ~> clearFStack(FStack)
%        ~> releaseExecutor
%        ~> enterState(Target | Args | Epoch +Int 1 )        @\label{lstline:enter-state}@
%       </k>
%       <env> Rho => .Map </env>
...
\section{Timer Semantics}\label{sec:timer-semantics}

Next, we discuss how \MediK{} handles temporal aspects of \BPGs{}. For instance, consider the
Fluid resuscitation guideline \BPG{} from \autoref{fig:fluid-therapy}.
...
in \MediK{}, \inlinemedik{sleep(duration)} is described by the rule in
\autoref{lst:sleep-rule}.
...
\begin{lstlisting}[
  ...
  ,numberstyle=\tiny
  ,xleftmargin=3ex,
  ,label={lst:sleep-rule}
  ,caption={Semantics of \inlinemedik{sleep}}
  ]
rule <k> sleep(Duration:Int) ;                    @\label{lstline:sleep-stmt}@
      =>    jsonWrite( { "action"   : "sleep"     @\label{lstline:json-start}@
                       , "duration" : Duration    @\label{lstline:sleep-duration}@
                       , "tid"      : TId }       @\label{lstline:transaction-id}@
                       , ... )                    @\label{lstline:json-end}@
         ~> releaseExecutor                       @\label{lstline:context-switch}@
         ~> waitForSleepResponse(TId) ...         @\label{lstline:wait-for-response}@
     </k>
     <tidCount> TId => TId +Int 1 </tidCount>     @\label{lstline:tid-increment}
\end{lstlisting}

The \inlinemedik{sleep} statement results in a JSON message being sent to a remote
endpoint (Lines \ref{lstline:json-start}-\ref{lstline:json-end})
specified when the \MediK{} process is launched, mimicing
...
once the desired duration has passed. A unique transaction-id
(Lines \ref{lstline:transaction-id}, \ref{lstline:wait-for-response} and \ref{lstline:tid-increment}),
...
\inlinemedik{sleep} causes a context-switch to occur on \autoref{lstline:context-switch},
releasing the executor lock to process other \emph{scheduled} machines.
...
the corresponding event signaling the completion of the sleep statement
...
The rule that handles the external response is shown in \autoref{lst:sleep-completion}.
\begin{lstlisting}[
  float=ht
  ,frame=single
  ,style=ksty
  ...
  ,numberstyle=\tiny
  ,xleftmargin=3ex,
  ,caption={Sleep Completion}
  ,label={lst:sleep-completion}
]
rule <k> waitForSleepResponse(TId) => . ... </k> @\label{lstline:sleep-wait}@
     <inBuffer> @\label{lstline:in-buffer-begin}@
        (ListItem(event($SleepDone | TId | Tau )) => .List)  ... @\label{lstline:external-response}@
      </inBuffer> @\label{lstline:in-buffer-end}@
     <executorAvailable> true => false </executorAvailable> @\label{lstline:execution-lock}@
     <epoch> Epoch </epoch> @\label{lstline:current-epoch}@
  requires Tau <=Int Epoch @\label{lstline:epoch-condition}@
\end{lstlisting}
The \inlinemedik{waitForSleepResponse(TId)} (\autoref{lstline:sleep-wait})
blocks execution until the external response indicating
the expiration of the sleep timer is received in the input buffer (Line \ref{lstline:in-buffer-begin}-\ref{lstline:in-buffer-end}).
Once the response is received, the machine instance resumes execution when
\begin{enumerate*}[label=(\alph*)]
  \item the execution lock becomes available
    (indicated by \inlinemedik{true} on \autoref{lstline:execution-lock}), and,
  \item the epoch the instance was scheduled to resume execution in (\autoref{lstline:external-response})
    is less than or equal to the current epoch (Lines \ref{lstline:current-epoch}-\ref{lstline:epoch-condition}).
\end{enumerate*}

An \inlinemedik{obtainFrom} statement also results in a context switch as
shown in \autoref{lst:obtain-from-request}.
Just as in the case of sleep, a JSON message is sent to the remote
endpoint with the \emph{name} of the parameter being requested and a unique transaction
(Lines \ref{lstline:obtain-from-begin}-\ref{lstline:obtain-from-end}),
while the machine instance releases the execution lock (\autoref{lstline:obtain-from-release}),
and waits for a response, as shown in \autoref{lst:obtain-from-request}.
...
\begin{lstlisting}[
  ...
  ,numberstyle=\tiny
  ,xleftmargin=3ex,
  ,caption={Obtain From Request}
  ,label={lst:obtain-from-request}
]
rule <k> obtainFrom(instance(Id:Int), Field:String)
      =>   jsonWrite( { "name"      : "Obtain"                     @\label{lstline:obtain-from-begin}@
                      , "args"      : [ Field , .JSONs ] } ...)    @\label{lstline:obtain-from-end}@
        ~> releaseExecutor                                         @\label{lstline:obtain-from-release}@
        ~> waitForObtainResponse(TId) ...
     </k>
     <tidCount> TId => TId +Int 1 </tidCount>
\end{lstlisting}

As depicted in \autoref{lst:obtain-from-response},
once data for the requested field is available, it's communicated as an
event to the \MediK{} process as a JSON message, where the response
consists of a JSON message with the requested data and the unique \inlinemedik{transactionId},
placed at the \emph{head} of the \inlinemedik{<inBuffer>} cell and scheduled for
execution in the next epoch (Lines \ref{lstline:response-buffer-begin}-\ref{lstline:response-buffer-end}).
...
\begin{lstlisting}[
  ...
  ,numberstyle=\tiny
  ,xleftmargin=3ex,
  ,caption={Obtain From Response}
  ,label={lst:obtain-from-response}
]
 rule <k> waitForObtainResponse(_) => V ... </k>
      <inBuffer> @\label{lstline:response-buffer-begin}@
        (ListItem(event($ObtainResponse | V:Val  | Epoch ))
        => .List)
        ...
      </inBuffer>                                                     @\label{lstline:response-buffer-end}@
      <executorAvailable> true => false </executorAvailable>
      <epoch> Epoch </epoch>
\end{lstlisting}

\section{\MediK{} Pediatric Sepsis System}\label{sec:medik-psepsis}
...
Recall from \autoref{fig:sepsis-screening} the guideline
for sepsis screening.
In \autoref{lst:medik-sepsis-screening}, we show
...
\ref{lstline:obtain-age-begin}-\ref{lstline:obtain-age-end}),
\inlinemedik{ObtainWeight} (\autoref{lstline:obtain-weight}), and
\inlinemedik{ObtainHighRiskConditions} (\autoref{lstline:obtain-hr-conditions}) in \autoref{lst:medik-sepsis-screening}.
Within each of these states, the code
permits communication with heterogeneous external agents for obtaining
required parameters. For instance, on \autoref{lstline:instruct-get-age}, an
...
runs on a tablet held by the Healthcare Practitioner and handles
...
step. Once all appropriate measurements have been obtained,
they are checked for abnormality (Lines \ref{lstline:calculate-score-begin}-\ref{lstline:calculate-score-end})
using tables shown in
to arrive upon a diagnosis.
...
\begin{lstlisting}[
  ...
  ,numberstyle=\tiny
  ,xleftmargin=3ex,
  ,caption={Sepsis Screening in \MediK{}}
  ,label={lst:medik-sepsis-screening}
]
machine SepsisScreening receives .. {
  ...
  state ObtainAge {                         @\label{lstline:obtain-age-begin}@
    entry {
      send tablet, Instruct, ("get age");   @\label{lstline:instruct-get-age}@
    } on ConfirmAgeEntered do {
      goto ObtainWeight;
    }
  }                                         @\label{lstline:obtain-age-end}@
  state ObtainWeight { ... }                @\label{lstline:obtain-weight}@
  state ObtainHighRiskConditions { ... }    @\label{lstline:obtain-hr-conditions}@
  state CalculateScore {                    @\label{lstline:calculate-score-begin}@
      var hrAbnormal = !isInNormalRange("HR", ...); @\label{lstline:isInNormalRange}@
      ...
      send tablet, SepsisDiagnosis
        , (sepsisSuspected);
  }                                         @\label{lstline:calculate-score-end}@
}
\end{lstlisting}
...
uses them to evaluate the patient for sepsis. For instance, on
\autoref{lstline:instruct-get-age},
...
meets the criteria for sepsis, we use functions, such as the one on \autoref{lstline:isInNormalRange},
...
\begin{lstlisting}[
  ...
  ,numberstyle=\tiny
  ,xleftmargin=3ex,
  ,caption={Fluid Therapy Guidelines}
  ,label={lst:medik-fluid-therapy}
]
machine FluidTherapy
  receives StartFluidTherapy, ... {

  init state Start {
    on StartFluidTherapy do {                           @\label{lstline:start-fluid-therapy}@
      goto ObtainRisks;
    }
  }
  state ObtainRisks {                                   @\label{lstline:obtain-risks-begin}@
    // Obtain fluid overload related risks
  }                                                     @\label{lstline:obtain-risks-end}@
  state SuggestFluidDosage {                            @\label{lstline:suggest-dose-begin}@
    // Suggest a dosage based on risks
  }                                                     @\label{lstline:suggest-dose-end}@
  state WaitForAdministerFluidConfirmation {            @\label{lstline:hcp-confirm-dose-begin}@
    // Handler for Normal Saline Administration
    on ConfirmNormalSalineAdministered do {
      sleep(900);                                       @\label{lstline:wait}@
      goto EvaluateResponsiveness;
    }
  }                                                     @\label{lstline:hcp-confirm-dose-end}@
  state EvaluateResponsiveness {
    entry {                                             @\label{lstline:evaluate-responsiveness-begin}@
      send tablet
        , Instruct
    ...
    }                                                   @\label{lstline:evaluate-responsiveness-end}@
  }
  state ObtainFluidOverloadSigns {                      @\label{lstline:evaluate-overload-begin}@
    // Obtain signs of fluid overload
  }                                                     @\label{lstline:evaluate-overload-end}@
  state AskNextStep {
    entry {
      var recommendation;
      if (this.fluidOverload) {
        recommendation = "handle fluid overload";       @\label{lstline:handle-overload}@
      } else {
        // obtain total saline dose                     @\label{lstline:obtain-fluid-dose}@
        if ((totalSalineDose >
              measurementBounds.salineDosageUpperBound) {
          if (isResponseiveToFluids) {
            recommendation = "maintainence fluids"      @\label{lstline:maintain-fluid}@
          } else {                                      @\label{lstline:inotropic-support-begin}@
            recommendation = "consider inotropic support";
            broadcast ConsiderInotropicSupport;
          }                                             @\label{lstline:inotropic-support-end}@
        } else {                                        @\label{lstline:more-bolus-begin}@
          recommendation = "repeat fluid bolus";
        }                                               @\label{lstline:more-bolus-end}@
      }
...
Recall from \autoref{sec:bpg-background} that once a sepsis diagnosis has been arrived upon,
one of the guideline suggested actions include administering fluids as shown in
\autoref{fig:fluid-therapy}. In \autoref{lst:medik-fluid-therapy},
...
corresponding to a button press by the \HCP{} is received (\autoref{lstline:start-fluid-therapy}).
The next steps include:
\begin{enumerate}[label=(\alph*)]
  \item Obtaining any \emph{risks} associated with administering
    fluids (Lines \ref{lstline:obtain-risks-begin}-\ref{lstline:obtain-risks-end}).
  \item Suggesting an appropriate \emph{dose} to administer based
    on the risks, if any (Lines \ref{lstline:suggest-dose-begin}-\ref{lstline:suggest-dose-end}).
  \item Waiting for the \HCP{} to confirm that the suggested
    dose was administered (Lines \ref{lstline:hcp-confirm-dose-begin}-\ref{lstline:hcp-confirm-dose-end}).
\end{enumerate}
Once the dose is administered, the machine waits for the
for 15 minutes as specified by the guideline (\autoref{lstline:wait}), before prompting
the \HCP{} to evaluate the patient's responsiveness to the administered fluid
dose (Lines \ref{lstline:evaluate-responsiveness-begin}-\ref{lstline:evaluate-responsiveness-end}),
and check for any signs of evaluate overload (Lines \ref{lstline:evaluate-overload-begin}-\ref{lstline:evaluate-overload-end}).
If the patient exhibits any signs of fluid overload, then a recommendation
to handle the overload is made (\autoref{lstline:handle-overload}). Otherwise,
the total dose of administered fluid is obtained from an external source (\autoref{lstline:obtain-fluid-dose}).
...
\begin{enumerate*}[label=(\alph*)]
  \item reduce the fluid flow to maintenance levels (\autoref{lstline:maintain-fluid}), or,
  \item switch to inotropic support to address circulatory issues is made (Lines \ref{lstline:inotropic-support-begin}-\ref{lstline:inotropic-support-end}).
\end{enumerate*}
If the total dose of administered
fluids is less than the maximum allowed limit, then a recommendation to
administer one more fluid bolus made (Lines \ref{lstline:more-bolus-begin}-\ref{lstline:more-bolus-end}) ).

Note that both the \inlinemedik{SepsisScreening} and
\inlinemedik{FluidTherapy} machines structurally resemble their
paper based counterparts in \autoref{fig:sepsis-screening} and
\autoref{fig:fluid-therapy} respectively, making
it easier for Healthcare Providers to comprehend and validate
the code.

\subsection{Formal Analysis using \MediK{}}\label{sec:formal-analysis}
...
Due to this, ghost machines may have statements to express non-determinism
in processes. Consider, for instance, on a positive sepsis diagnosis, an
\HCP{} may choose to either administer fluids first, followed by antibiotics, or
vice-versa. \MediK{} supports such non-determinism using \inlinemedik{either-or}
statements as shown in \autoref{lst:non-determinism}.
...
\begin{lstlisting}[
  ...
  ,numberstyle=\tiny
  ,xleftmargin=3ex,
  ,caption={Modeling Non-Determinism}
  ,label={lst:non-determinism}
]
either {
...
that have semantics shown in \autoref{lst:abstract-semantics}.
...
Recall from Section \ref{sec:bpg-background} that we needed to:
\begin{enumerate*}[label=(\arabic*)]
...
is performed using the \inlinemedik{in-interval} construct as shown in \autoref{lst:hr-table}.
Recall that the \inlinemedik{in-interval} construct is merely syntactic
...
result of this abnormality checking operation is an abstract boolean value. Thus,
instead of exploring each branch of \inlinemedik{if-else} statements
corresponding to \inlinemedik{in-interval} constructs in \autoref{lst:hr-table}, we replace the entire checking process with a
...
values. We then added the rule in \autoref{lst:stuck-rule}, that takes
...
\begin{lstlisting}[
  ...
  ,numberstyle=\tiny
  ,xleftmargin=3ex,
  ,label={lst:stuck-rule}
  ,caption={Unresponsive Machine Transition}
  ]
...
\begin{figure}[b!]
  \centering
  \includegraphics[width=0.65\textwidth]{medik-pyramid}
  \caption{Our Contribution}\label{fig:our-contribution}
\end{figure}

In \autoref{sec:sepsis-bpg}, we described several characteristics
of \BPGs{} that a language for expressing computer interpretable
best practice guidelines should ideally accommodate. These include
\begin{enumerate*}[label=(\roman*)]
  \item comprehensibility for healthcare providers to enable them to
    verify correctness of medical logic,
  \item ability to accommodate diverse external agents,
  \item execution and analysis tools with correctness guarantees.
\end{enumerate*}

In \autoref{chapter:related-work}, we discussed progress made by existing approaches
towards addressing them. We measured progress along the following lines:
\begin{itemize}
  \item \textbf{Implementation-Specification Gap:} The textual \BPG{}
    can also be used \say{as-is} in a \CDSS{} implementation, eliminating
    any translation from a textual document to its computable encoding.
  \item \textbf{Formal Semantics:} The language has a complete
    formally defined semantics that serves as a manual for implementing
    tools and associated ecosystem.
  \item \textbf{Formal Analysis Tools:} Existence of a comprehensive set
    of analysis tools that can be used to analyze \BPGs{} implemented in the
    language that evolve as the language evolves.
  \item \textbf{Holistic Safety:} Comprehensive support for formal analysis
    and tools, including execution engines, with correctness guarantees.
\end{itemize}


When compared to existing approaches from \autoref{chapter:related-work},
\MediK{} is the first computer-interpretable guideline language that has
a complete executable semantics. As \MediK{}'s tools are derived from its
semantics, they are, by definition, correct-by-construction. As the tools
don't have to be maintained independently, updates to the language to
accommodate \HCP{} feedback only require corresponding changes to the
semantics, and tools evolve automatically.
  ...
  \end{tabularx}
    \caption{\MediK{} vs Existing Approaches}\label{table:medik-comparison}
  \end{table}



This work attempted to establish that the semantics-first approach
embodied by \MediK{} makes building safer \CDSSs{} easier.
We show that \MediK{}'s communicating state machines-based approach provides a
convenient medium for expressing medical knowledge by utilizing
it to build a real-world complex \CDSS{} for management of
sepsis in pediatric cases in collaboration with OSF St. Francis
Medical Center. \MediK{}'s support for modeling external agents as
\emph{ghosts} provides a uniform way of formal analysis. We use this
to model-check the sepsis \CDSSs{} for responsiveness, To the best of our knowledge,
knowledge, our is the first system for sepsis management with any form of formal
analysis.
We briefly summarize how \MediK{} compares against existing approaches in \autoref{table:medik-comparison}.

In \autoref{chapter:semantics-first-cdss}, we described incremental progress made
by existing approaches towards enabling greater \CDSS{} adoption through
\emph{solution themes} discussed in \autoref{sec:related-work-discussion} and
shown in \autoref{fig:our-contribution}. Specifically, upper rungs of the
pyramid incorporates features and learnings of the lower rungs. Our contribution
can be described by the run marked in red, as our work incorporates the ability
to implement \HCP{}-comprehensible, computable \BPGs{} of earlier approaches
with a complete formal semantics, comprehensive support for formal analysis,
and correctness guarantees for all tools.


%Guideline-based Clinical Decision Support Systems (CDSSs)
%are now considered vital to the future of Medical Decision making in general,
%But, to find widespread adoption, guideline-based \CDSSs{} must be
%held to the highest standards for safety-critical systems. While
%several advances have been made to \CDSS{} over the years, several
%limitations have also been identified. This work fixed said
%limitations by introducing \MediK{} - a new \DSL{} for expressing
%\BPGs{} that uses a semantics-first approach to build \CDSS.
%\MediK{} programs consist of \emph{concurrently} executing \emph{instances}
%of \emph{State Machine}. \MediK{} models
%external agents as machines with \emph{transition systems}
%external to the program called \emph{interfaces}, allowing for a \emph{uniform} way of dealing with \emph{heterogenous external agents}. For program analysis, \MediK{} allows
%modeling external agents via ghost machines that support
%\emph{non-determinism}, enabling model-checking \CDSSs{} for responsiveness.
%We collaborated with the Children's Hospital of Illinois at OSF St. Francis
%Medical Center to develop
%a system for screening and management of pediatric sepsis using \MediK, and
%demonstrated it satisfies desired safety properties. To the best of our
%knowledge, our is the first system for sepsis management with any formal safety
%guarantees.
%
