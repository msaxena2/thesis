...
Recall that on execution, $\K{}$ replaces \inlinek{\$PGM} (\autoref{lstline:k-cell})
...
defining \BPGs{} using their corresponding $\K$-rules.
...
The \K{} variable \inlinek{MName} matches the name of the machine whose instance is being created.
The rule then looks for an \inlinek{<machine>} sub-configuration that
has the same name, i.e., the name that variable \inlinek{MName} matches
on \autoref{lstline:source-k-cell} to obtain the instance variable
declarations, matched by variable \inlinek{MachineDecls} on \autoref{lstline:declaration-code}
and the state prefixed with the keyword \inlinek{init} (Lines \ref{lstline:init-state-begin}-\ref{lstline:init-state-end}).
Then a new \inlinek{<instance>} cell is added to the configuration (Lines \ref{lstline:new-instance-begin}-\ref{lstline:new-instance-end}),
and its unique id, stored under the \inlinek{<id>} cell, is added to the set of
active instances (\autoref{lstline:active-instances-update}).
...
\begin{lstlisting}[
  ...
  numbersep=5pt,
  xleftmargin=3ex,
  caption={Creating Instances via the \inlinek{new} Operator},
  label={lst:new-rule}
]
rule <id> SourceId </id>
