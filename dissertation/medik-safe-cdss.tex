...
Recall from \autoref{sec:semantics-in-k} that $\K$ uses the \BNF{} grammar
to generate a parser for the language. The
Semantics has two components:
...
Recall that on execution, $\K$ replaces \inlinek{$PGM}
with the Abstract Syntax Tree (AST)
of the program, obtained by parsing the program using the language's syntax definition.
As \inlinek{$PGM} in \MediK{} exists under

%$\K{}$ replaces \inlinek{$PGM} (\autoref{lstline:k-cell})
with the Abstract Syntax Tree (AST)
...
\inlinek{<instance>} cell that has a \inlinek{<k>} cell with
\inlinek{populateCells($PGM) ~> createInitInstances} at its top
...
These cells are utilized in implementing the scheduling strategy \MediK{}
...
%then one rule is non-deterministically chosen and applied.
An execution is a
