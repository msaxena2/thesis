...
the semantics-first approach discussed in \autoref{sec:semantics-first-cdss}.
Thus, it has:
\begin{itemize}
...
\subsection{\HCP{} Comphrenesibility}
...
among which is the factor that \HCPs{} are typically not trained to understand
conventionally programming languages. To address this, we designed \MediK{}
s.t. encoded guidelines resemble their physical, non-executable counterparts.

From \autoref{sec:}\BPGs{} typically involve concurrent workflows,
often expressed using a flowchart-like notation that may involve
inter-workflow interactions. In \autoref{section:kacls-backend},
...
Recall from \autoref{chapter:semantics-in-k}, the $\K{}$ definition
