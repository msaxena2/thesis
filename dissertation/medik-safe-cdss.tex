...
we recall the example \BPG{} from section \autoref{sec:background}
...
In section \autoref{sec:related-works}, we discuss how \MediK{} builds on
...
intravenous (IV) fluids is \emph{vital} is \emph{vital} for avoiding
adverse outcomes \cite{Weiss2014CCM,Evans2018JAMA}. The
\BPG{} has several concurrent workflows with inter-workflow
dependencies, and can be used to study characteristics common to
all \BPGs{}. We listed these in detail in \autoref{sec:bpg-background},
and recall them here.
...
%and to illustrate characteristics that are desired of a \DSL{} for such systems.
%
%Sepsis is life-threatening condition caused by the body's extreme response to
...
In this section, we introduce the \MediK{} \DSL{} for expressing \CIGs{}.
\MediK{} has designed to describe \BPGLogic{} used in
safety-critical systems. Thus, it is vital that:
\begin{itemize}
  \item The interpreter is correct w.r.t. the formal semantics.
  \item The language has a comprehensive suite of formal program analysis tools.
  \item New features based on \HCP{} feedback can be
    implemented quickly, conveniently, and correctly.
\end{itemize}
...
The remainder of this section introduces \MediK{} and describes how it's
designed around characteristics of \BPGs{} from Section \ref{sec:motivating-example}.
...
to \HCPs{}, they must be representable in a flowchart-like notation that \HCPs{}
are already comfortable with, and be capable of expressing inter-workflow
interactions succinctly. To address these diverse requirements, we borrow from
existing state-of-art languages for modeling large concurrent
...
%validating \BPGs{} easier. We explore the differences
%to existing techniques in section \ref{sec:related-works}.
%In \MediK{}, programs are expressed as concurrently
%executing instances of state machines that communicate via passing messages.
%Given a \BPG{} where each workflow is expressed as a flowchart,
%we express said flowcharts as State Machines in \MediK{}. During execution,
%instances of these machines are created, that interact with each other by
%passing events. Note the distinction between
%machine and its instance. A machine
%is analogous to an Object Oriented Programming (OOP) class, whereas
%its instance is analogous to an OOP object.

Next, we describe \MediK{} using its $\K$-framework definition. The $\K{}$ definition
...
We use the skeleton of a \MediK{} machine below, and use
...
%\begin{itemize}
%  \item (\textbf{lines 1-2}) The keyword \inlinemedik{machine}, followed by the name
%    and a comma-separated list of identifiers signifying events that
%    it \inlinemedik{receives} via the broadcast mechanism.
%    One machine in a program can be
%    prefixed with the \inlinemedik{init} keyword. On execution, an implicit instance
%    of this machine the created, and the \inlinemedik{entry} block of
%    initial state executed.
%  \item (line 3) A set of instance variables.
%  \item (\textbf{lines 5-11}) A set of state declarations. Each
%    state has a name, an optional entry block (\textbf{lines 6-8}),
%    and a set of event handlers (\textbf{lines 9-11}). The entry block
%    begins with the keyword \inlinemedik{entry}, and may contain a list of variables
...
Within the entry and event handler code blocks, there may be
statements. Below, we give a simplified version
of the $\K$-grammar for statements. In $\K$, productions
are defined using the keyword \inlinek{syntax} (lines 1, 7).
...
Lines 1-5 define the syntax of \MediK{} expressions.
Line 1 defines basic expressions such as
identifiers (denoted by the builtin $\K{}$ production \inlinek{Id}),
values such as booleans, or rationals, or \say{\inlinek{this}}, which enables
an instance to refer to itself. Line 2 defines the usual dot operator (\inlinek{.}),
which can be used to access members of an instance. \inlinek{obtainFrom} (line 3),
\inlinek{interval} (line 4) and \inlinek{in} (line 5) are useful in context of
defining \BPGs{}, and are described through an example in section \ref{sec:evaluation}.
Apart from these, \MediK{} also supports common expressions such as \inlinek{+,
-. >, >=} over rationals and \inlinek{&&, ||} over booleans.
