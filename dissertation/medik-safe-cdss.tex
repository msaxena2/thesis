...
using the \inlinemedik{broacast} construct described in \autoref{sec:messaging}.
...
the default clause (\autoref{lstline:default-clause}) is run.
...
$\textit{execute}(\Instance_{\Machine_{i},j}^{\tau}, \scheduled{})$ on Line \autoref{algline:execute}
...
specified when the \MediK{} process is launched. This mimics
...
consists of a JSON message with the requested data and the unqiue \inlinemedik{transactionId},
...
corresponding to \inlinemedik{in-interval} constructs in \autoref{lst:hr-table}, we replace the entire checking process with an
...
analysis.
We be briefly summarize how \MediK{} compares against existing approaches in \autoref{table:medik-comparison}.

In \autoref{chapter:semantics-first-cdss}, we described incremental progress made
by existing approaches towards enabling greater \CDSS{} adoption through the
use of \autoref{fig:our-contribution}. Specifically, upper rungs of the
pyramid incorporate features and learnings of the lower rungs. Our contribution
