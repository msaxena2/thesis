...
Recall from \autoref{sec:} \BPGs{} typically involve concurrent workflows,
...
We use the skeleton of a \MediK{} machine in \autoref{lst:machine-syntax}, and use
...
\begin{lstlisting}[
  ...
  numbers=left,
  numberstyle=\tiny,
  caption={\MediK{} Machine Skeleton},
  label={lst:medik-skeleton},
  xleftmargin=3ex
]
...
\autoref{lst:machine-definition} shows the skeleton of a machine
definition. It consists of:
\begin{itemize}
  \item (Lines \ref{lstline:machine-declaration}-\ref{lst:lstline:machine-receives})
...
statements. A simplified version of the grammar for statements is
shown in \autoref{lst:medik-grammar}.
Recall from \autoref{sec:semantics-in-k},
in $\K$, productions
