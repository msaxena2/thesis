...
\begin{enumerate}[label=(\roman*)]
  \item obtains the \inlinek{id} of the receiver instance,
    ...
    against the current configuration (\autoref{lstline:k-send}),
  ...
  \item adds the event and associated arguments to the
    ...
    with \inlinek{id} \inlinek{RecvId} (Lines \ref{lstline:instance-b-begin}-\ref{lstline:isntance-b-end}).
...
to how other languages, such as Object-Oriented Maude,
handle external agents \cite{OOPMaudeUrl}. These
external agents are modeled in \MediK{} as \inlinemedik{interface}s.
An interface is simply a state machine that has its
transition system defined externally.
It is common to obtain certain measurements,
such as the patient's most recent heart rate, from sensors attached to
the patient. \autoref{lst:external-interaction} depicts one
way of connecting the sensor.
