...
laid out a roadmap to optimize \CDSS{} uptake in medicine. According to the
...
to address aforementioned challenges. We describe this progress
...
be understood by \HCPs{} \cite{SchiffmanYMI13}. To build a \CDSS{}, the \BPG{}
must be
...
Typically, to translate textual guidelines to a computable
...
For instance, in the case of EvidencePoint,
...
that permit encoding complex guidelines using a multi-layer
...
Therefore, an interpreter and analysis tools must be implemented in an
ad-hoc manner.

Several endeavors, mainly academic, but also involving the industry
towards making \CDSSs{} safer, easier, effective and cheaper.
We briefly talk about some related approaches in this chapter to
provide an overview of existing progress and remaining questions
but discuss them in greater detail in \autoref{chapter:related-work}, along with
other related work.
In \autoref{sec:monolithic-cdss},
...
\begin{itemize}
  \item Interpreters and compilers for said \DSLs{} are developed in an
    ad-hoc way and remain prone to bugs that may manifest during execution.
...
Addressing these issues is vital to improving \CDSS{} adoption.
To have trustworthy and useable computer interpretable \BPGs{},
