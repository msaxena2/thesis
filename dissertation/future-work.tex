...
Given configuration terms $\tau_1, \tau_2 \in
\Abs$ with \inlinek{<store>} cells containing maps $\rho_{\tau_1}$ and
$\rho_{\tau_2}$ respectively. We say $\tau_1 \sqsubseteq \tau_2$ iff
they only differ in their
\inlinek{<store>} cell (all other cells are similar), and, for every
pointer in $p \in$ \inlinekmath{keys($\rho_{\tau_1} \cup \rho_{\tau_2}$)}
we have $\rho_{\tau_1}[p] \sqsubseteq \rho_{\tau_2}[p]$. Note we
say $\rho[p] = \bot$ for any $p \not\in$ \inlinekmath{keys($\rho$)}.
