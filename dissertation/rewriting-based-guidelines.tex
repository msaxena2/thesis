...
\begin{subfigure}{\textwidth}
  \centering
  \includegraphics[width=0.60\linewidth]{cpr-machine}
  \caption{CPR Machine}
...
\begin{subfigure}{\textwidth}
  \centering
  \includegraphics[width=0.60\linewidth]{epi-machine}
...
state machines in a $\K$ definition.
...
\paragraph{CPR Machine} \hfill \break

The \textit{CPR Machine} encodes the intended CPR procedure
shown in figure \ref{fig:cpr-machine}. When the user clicks
the \say{Start} button in the CPR pane in figure \ref{fig:kacls-snapshot},
a two minute timer is started. If the user decides to stop CPR before
...
The following rule is trigger when the \say{Start} button in clicked:
\begin{lstlisting}[float=ht,
  frame=single,
  style=ksty,
  language=k,
  ...
  label={lst:cpr-machine},
  xleftmargin=3ex
]
  rule <machine>
            <id> cprMachine </id>
            <state> idle => lessThan2Min </state>
            <store> ( "cprRunning" |-> (_ => true) ) ... </store>
            ...
       </machine>
       <inputBuffer> "StartCpr" , IN => IN </inputBuffer>
       <outputBuffer>
...
\begin{enumerate*}[label=(\alph*)]
  \item consumes the \inlinek{"StartCpr"} event from the \inlinek{<inputBuffer>} in
    line 9,
  \item changes the state of the machine from \inlinek{idle} to \inlinek{lessThan2Min}
    in line 3 and,
  \item Sends a \inlinek{startTimer} action to the frontend via the
    \inlinek{<outputBuffer>} (encoded as JSON),
    which results in the start of a two minute timer in the frontend.
\end{enumerate*}
The rule uses several $\K$ features that greatly simplify the definitions.
$\K$'s \textit{configuration abstraction} mechanism allows user to \textit{only}
specify parts of the configuration needed in the rule. The
\inlinek{...} (dots) on lines 4 and 5 are used to ignore parts contents of a cell
not used in the rule. Another important feature is $\K$'s support for \textit{local
rewrites}, which makes it easy to rewrite subterms in deeply nested configurations.

\paragraph{Epinephrine Machine} \hfill \break
...
Next we show the rule formalizing the transition between \inlinek{givenLessthan3Min} and \inlinek{earlyOrderWarningNotification}.
The rule fires when it has been less than three minutes before the last dose and a fresh order for
Epinephrine. Since the guidelines dictate that Epinephrine must be administered
every three minutes, ordering fresh Epinephrine is a deviation from the
guidelines. Thus, an appropriate warning message is presented to the user.
\begin{lstlisting}[float=ht!,
  frame=single,
  style=ksty,
  language=k,
...
\paragraph{Shock Machine} \hfill \break
...
\end{enumerate*}
In case the of deviation appropriate warnings are issued.
\begin{lstlisting}[
  frame=single,
  style=ksty,
  language=k,
