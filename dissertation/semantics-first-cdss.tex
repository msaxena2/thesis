...
where the semantics \inlinek{module} \inlinek{imports}
the syntax \inlinek{module}. This is by convention, and
...
\begin{enumerate}[label=\alph*)]
  ...
  \item With the syntax and semantics residing in separate modules,
    $\K$ can be instructed to use only the syntax \inlinek{module} for parsing programs.
    This allows users to define additional syntactic constructs in the semantics module
    for use in rules. If a combined syntax and semantics \inlinek{module} is used instead,
...
To impart semantics, we must describe how the
evolution of the program's \emph{state} during
execution. This is done through \emph{rewrite rules} that operate over
$\K$-\emph{configurations}.
In $\K$ \emph{state} is organized into a \emph{configuration}.
