...
of the addition rule, $\K$ can \emph{infer} that sorts for variables
\inlinek{I1} and \inlinek{I2} to be \inlinek{Int}, as operands of the
...
Next, we consider the rule for program variable assignment on
\autoref{lstline:imp-assgn-rule}.
...
\emph{unchanged} during the rewrite. Since variable \inlinek{I} is suffixed with the sort
\inlinek{Int}, the variable assignment rule will \emph{not} match the
expression \inlinek{2 + 3}.
Recall the attribute \inlinek{strict(2)} from \autoref{lstline:imp-stmt-assgn} in
\autoref{lst:imp-syntax}), denoting
$\K$ will \emph{heat}, or pull-out the second
argument for evaluation. This results in a configuration term of the form
\inlinek{<k> 2 + 3} $\curvearrowright$ \inlinek{ x =\ }\hole\inlinek{ ; ...</k>},
where in $\K{}$ $\curvearrowright$ means \emph{followed-by}, i.e.,
the evaluation of \inlinek{2 + 3} must occur \emph{before} the evaluation
of \inlinek{x = \ }\hole\inlinek{;}. \hole
denotes a \emph{hole} left in place of the argument that was \emph{heated}.
\inlinek{<k> 2 + 3} $\curvearrowright$ \inlinek{x =\ }
\hole\inlinek{ ; ...</k>} is re-written to \inlinek{<k> 5} $\curvearrowright$
\inlinek{x =\ }\hole\inlinek{ ; ...</k>}
by an application of the arithmetic addition rule on line 29. On
