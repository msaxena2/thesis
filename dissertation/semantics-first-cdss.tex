...
A \emph{(matching logic) signature} $\MLSigma = \left(\EV,\SV, \Sigma\right)$
where $\Sigma$ is a set of symbols denoted $\sigma_1,\sigma_2,\dots$.
A \emph{(matching logic) pattern} $\varphi$ is inductively defined as:
\begin{align*}
\varphi \Coloneqq\
& x
\mid X
...
Given signature $\MLSigma$, a $\MLSigma$-model  $\mathbb{M} = \left(M, \_.\_,
...
Given a $\MLSigma$-model $M$, an $M$-valuation of variables
