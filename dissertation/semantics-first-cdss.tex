\chapter{Semantics-First Approach to Clinical Decision Support}\label{chapter:semantics-first-cdss}
...
(Lines \ref{lstline:imp-stmt-block}-\ref{lstline:imp-stmt-while}), that define
...
definition of a list of identifiers (\inlinek{Ids}) on \autoref{lstline:imp-ids}. In $\K{}$,
...
components under \inlinek{module IMP}.
...
\autoref{lst:init-configuration}, where \inlinek{.Ids} is the identity element for the user-defined
...
\inlinekmath{(X $\ \mapsto\ $ x, Xs\ $ \mapsto\ $ .Ids, Rho $\ \mapsto\ $.Map)}
...
\noindent Note the following conveniences that \K{} offers to make writing rules easier:
\begin{enumerate}[label=\roman*)]
...
on \autoref{lstline:imp-vardec-requires} of \autoref{lst:imp-semantics}. This forces the rule to only apply if
...
re-declared.
Next, the rule on \autoref{lstline:imp-emptydec-rule} of
\autoref{lst:imp-semantics} will eliminate
\inlinek{int .Ids;}, leaving the configuration shown in
\autoref{lst:id-elim-configuration} after the rewrite.
\begin{lstlisting}[float=h!,language=k,style=ksty,caption={Configuration Post
\inlinek{.Ids} Elimination},label=lst:id-elim-configuration]
<T>
  <k> x = 2 + 3; </k>
  <state> x |-> 0 </state>
</T>
...
Also note that the rule on \autoref{lstline:imp-emptydec-rule}
does not explicitly mention the cell on which
...
to \inlinek{5}, resulting in the final configuration shown in
\autoref{lst:final-configuration}.

\begin{lstlisting}[float=h!,language=k,style=ksty,frame=single,label=lst:final-configuration,caption={Final
Configuration}]
<T>
  <k> . </k>
  <state> x |-> 5 </state>
...
Also note the use of \inlinek{...} in the assignment rule. In $\K{}$,
...
\begin{lstlisting}[float=h!,language=k,style=ksty,frame=single,label=lst:kseq-configuration,caption={Configuration
...
\inlinek{.}), which leads to the configuration shown in
...
\subsection{Matching Logic: Theoretical Foundations of
...
Note that $\varphi\left[\psi / x\right]$ (respectively $\varphi\left[\psi / X\right]$) is the result of
substituting $\psi$ for $x$ (respectively $X$), where bound variables in
...
In \autoref{sec:semantics-first}, we outlined advantages of the
